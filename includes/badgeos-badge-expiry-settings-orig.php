  ...
  <script type="text/javascript">
    ...
    <table class="form-table">
      <tr valign="top">
        ...
        <td style="width:15%;">Validity 1</td>
        <td style="width:15%;">Validity 2</td>
        <td style="width:15%;">Last ladder change period</td>
        <td style="width:15%;">Send email<br />x days before expiry</td>
        <td></td>
      </tr>
    <?php 
        ...
        foreach ($badges as $badge) {
          $badges_expiry = isset($badges_expires[$badge->ID]) ? $badges_expires[$badge->ID] : new stdClass;
...
<?php
  ...
  protected function send_email_notification($user, $badge_obj, $validity_settings, $date_args) {
    ...
    if ($date_diff && $date_diff->format('%a') == 0) {
      $subject  = 'Badge Expiry';
      $message  = 'Your badge will expire on '.$date_args['expiry_date']->format('Y-m-d H:i:s');
  ...
  public function badge_epiry_cron_job() {
    ...
    foreach ($_badgeos_achievements as $_badgeos_achievement) {
      ...
      foreach (current($badges_arr) as $badge_obj) {
        ...
        $validity_settings  = isset($badges_expires[$post_id]) ? $badges_expires[$post_id] : array();
        //new dBug($validity_settings, '', 1);
